 ...
 * @param {number} [knownStock] - stock déjà connu après ajout (évite un re-read Firestore qui peut être en retard)
 */
async function checkAndSendReplenishmentAlert(bot, productId, productTitle, knownStock) {
  if (!STOCK_ALERT_ENABLED || !bot || !productId) return;
  const currentStock = knownStock != null && knownStock >= 0
    ? knownStock
    : await getAvailableCount(productId);
