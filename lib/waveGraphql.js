...
async function verifyTransactionIdInWave(transactionId, order) {
  try {
    ...
    const entryAmount = entry.grossAmount != null ? entry.grossAmount : entry.amount;
    if (orderAmount > 0 && entryAmount != null && Math.abs(Number(entryAmount) - orderAmount) > 1) {
      return { valid: false, message: `Le montant Wave (${entryAmount} F) ne correspond pas à la commande (${orderAmount} F). Vérifiez l'ID de transaction.` };
    }
